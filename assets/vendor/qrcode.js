/*
 * qrcode.js — compact QR Code generator
 * Ported/adapted from the public-domain "qrcode-generator" algorithm
 * (Kazuhiko Arase, MIT-style license: "no restriction"). Vendored here so
 * the RA9MANA site can render QR codes fully offline, with no external
 * script, API, or network call — consistent with the site's static,
 * no-third-party-dependency approach.
 *
 * Usage:
 *   const qr = qrcode(0, "M");      // typeNumber 0 = auto-detect size
 *   qr.addData("https://example.com");
 *   qr.make();
 *   const svg = qr.createSvgTag(4, 0); // cellSize, margin
 */
var qrcode = (function () {
  "use strict";
  ...
  QRCodeModel.prototype = {
    ...
    setupTypeInfo: function (test, maskPattern) {
      var data = (QRUtil.getErrorCorrectLevelValue(this.errorCorrectLevel) << 3) | maskPattern;
  ...
  for (qi = 8; qi < 256; qi++) {
    QRMath.EXP_TABLE[qi] = QRMath.EXP_TABLE[qi - 4] ^ QRMath.EXP_TABLE[qi - 5] ^ QRMath.EXP_TABLE[qi - 6] ^ QRMath.EXP_TABLE[qi - 8];
  }
  for (qi = 0; qi < 255; qi++) QRMath.LOG_TABLE[QRMath.EXP_TABLE[qi]] = qi;

  function QRPolynomial(num, shift) {
    if (num.length == null) throw new Error(num.length + "/" + shift);
    var offset = 0;
  ...
  QRRSBlock.getRsBlockTable = function (typeNumber, errorCorrectLevel) {
    var value = typeof errorCorrectLevel === "string" ? QRUtil.getErrorCorrectLevelValue(errorCorrectLevel) : errorCorrectLevel;
    switch (value) {
  ...
  var QRUtil = {
    ...
    getErrorCorrectLevelValue: function (level) { switch (level) { case "L": return 1; case "M": return 0; case "Q": return 3; case "H": return 2; default: throw new Error("bad level"); } },
  ...
  // Rough capacity table (byte mode) per type/ECL, used for type auto-detection.
  function getMinimumType(sLength, errorCorrectLevel) {
    for (var type = 1; type <= 40; type++) {
      var rsBlocks = QRRSBlock.getRSBlocks(type, errorCorrectLevel);
      var buffer = new QRBitBuffer();
      buffer.put(4, 4);
      buffer.put(sLength, QRUtil.getLengthInBits(4, type));
      var totalDataCount = 0;
      for (var i = 0; i < rsBlocks.length; i++) totalDataCount += rsBlocks[i].dataCount;
      var neededBits = 4 + QRUtil.getLengthInBits(4, type) + sLength * 8;
  ...
  return function (typeNumber, errorCorrectLevel) {
    ...
    qr.addData = function (data) { _data = String(data); };
    qr.make = function () {
      var type = typeNumber;
      if (!type) type = getMinimumType(byteLength(_data), errorCorrectLevel || "M");
      _model = new QRCodeModel(type, errorCorrectLevel || "M");
      _model.addData(_data);
      _model.make();
    };
    qr.getModuleCount = function () { return _model.getModuleCount(); };
    qr.isDark = function (row, col) { return _model.isDark(row, col); };

    // Renders the QR as a standalone SVG string (no external assets).
      ...
      for (var row = 0; row < count; row++) {
        for (var col = 0; col < count; col++) {
          if (_model.isDark(row, col)) {
            rects.push('<rect x="' + (col * cellSize + margin) + '" y="' + (row * cellSize + margin) + '" width="' + cellSize + '" height="' + cellSize + '"/>');
          }
        }
      }
      return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + size + ' ' + size + '" width="' + size + '" height="' + size + '" shape-rendering="crispEdges">' +
        '<rect width="' + size + '" height="' + size + '" fill="#ffffff"/>' +
        '<g fill="#0B2027">' + rects.join("") + '</g></svg>';
    };

    function byteLength(s) {
      // approximate UTF-8 byte length used for auto type sizing
      var b = 0;
      for (var i = 0; i < s.length; i++) {
        var code = s.charCodeAt(i);
        if (code > 0x10000) b += 4; else if (code > 0x800) b += 3; else if (code > 0x80) b += 2; else b += 1;
      }
      return b;
    }
